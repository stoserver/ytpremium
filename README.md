...
### 관리자 기능
- `/시작하기`: 봇 초기 설정 (최초 1회, 버튼 기반 설정, 관리자만 보임)
...
**버튼 기반 설정 절차** (관리자만 보임):
1️⃣ **환영 메시지** - "다음" 버튼을 클릭하여 시작
2️⃣ **구매자 역할 설정** - "새로 만들기" 또는 "기존 역할 사용" 버튼 선택
3️⃣ **로그 채널 설정** - "새로 만들기" 또는 "기존 채널 사용" 버튼 선택
4️⃣ **오너 역할 설정** - "역할 ID 입력하기" 버튼 클릭 후 ID 입력
5️⃣ **티켓 관리자 역할** - "역할 ID 입력하기" 버튼 클릭 후 ID 입력
6️⃣ **티켓 카테고리** - "새로 만들기", "기존 사용", "없음" 중 선택
7️⃣ **패널 채널** - "새로 만들기" 또는 "기존 채널 사용" 버튼 선택

모든 메시지는 **ephemeral**(나만 보기)로 표시되어 관리자만 볼 수 있습니다.
...
**참고:**
- 업데이트 중 중요 파일(.env, config.json, database.json)은 자동으로 백업됩니다
...
│   └── panel.js           # 패널 생성
├── index.js               # 메인 봇 파일
├── config.json            # 봇 설정 (자동 생성됨 - /시작하기로 생성)
...
`config.json` 파일은 `/시작하기` 명령어로 자동 생성됩니다.
...
**설정 항목:**
- `setupCompleted`: 초기 설정 완료 여부
- `buyerRoleId`: 구매자에게 부여할 역할 ID
- `logChannelId`: 봇 활동 로그를 기록할 채널 ID
- `panelChannelId`: 패널을 전송할 채널 ID
- `ownerRoleId`: 오너 역할 ID (티켓에서 멘션됨)
- `ticketManagerRoleId`: 티켓 관리자 역할 ID (티켓에서 멘션됨)
- `ticketCategoryId`: 티켓이 생성될 카테고리 ID (선택 사항)
